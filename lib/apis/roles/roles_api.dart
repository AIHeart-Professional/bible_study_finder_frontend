...
class RolesApi {
  ...
  static Future<bool> modifyGroupRoleApi(
      String userId, String groupId, String role) async {
    final stopwatch = Stopwatch()..start();
    final url = _buildModifyGroupRoleUrl();

    _logger.debug('Modifying group role: userId=$userId, groupId=$groupId, role=$role');

    try {
      final response = await _sendModifyGroupRoleRequest(url, userId, groupId, role);
      stopwatch.stop();
      _logApiCall('POST', url.toString(), response.statusCode, stopwatch);
      return _handleModifyGroupRoleResponse(response);
    } catch (e, stackTrace) {
      stopwatch.stop();
      _logApiError('POST', url.toString(), e);
      _logger.error('Error modifying group role', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  static Uri _buildModifyGroupRoleUrl() {
    return Uri.parse('${AppConfig.getBackendBaseUrl()}/roles/modify_group_role');
  }
  ...
  static Future<http.Response> _sendModifyGroupRoleRequest(
      Uri url, String userId, String groupId, String role) async {
    final authHeaders = await AuthStorage.getAuthHeaders();
    final headers = {
      ...authHeaders,
  ...
  static bool _handleModifyGroupRoleResponse(http.Response response) {
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      final success = jsonData['success'] ?? false;
      if (success) {
        _logger.debug('Successfully modified group role');
        return true;
      } else {
        final message = jsonData['message'] ?? 'Failed to modify group role';
        _logger.warning('API returned success=false: $message');
        return false;
      }
    } else {
