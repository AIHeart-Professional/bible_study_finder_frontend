...
class GroupService {
  ...
  static Future<List<GroupMember>> getGroupMembers(String groupId) async {
    _logger.debug('getGroupMembers called with groupId=$groupId');
    try {
      final members = await GroupApi.getGroupMembersApi(groupId);
      _logger.info('Successfully loaded ${members.length} group members');
      return members;
    } catch (e, stackTrace) {
  ...
  static Future<bool> createGroupRequest(
      String groupId, String requestMessage) async {
    _logger.debug(
        'createGroupRequest called with groupId=$groupId, message=$requestMessage');
    try {
      final userId = await AuthStorage.getUserId();
      if (userId == null) {
        _logger.warning('User not logged in, cannot create group request');
        throw Exception('User not logged in');
      }

      final success = await GroupApi.createGroupRequestApi(
          groupId, userId, requestMessage);
      if (success) {
        _logger.info('Successfully created group request for group: $groupId');
      } else {
        _logger.warning('Failed to create group request');
      }
      return success;
    } catch (e, stackTrace) {
  ...
  static Future<List<GroupRequest>> getGroupRequests(String groupId) async {
    _logger.debug('getGroupRequests called with groupId=$groupId');
    try {
      final requests = await GroupApi.getGroupRequestsApi(groupId);
      _logger.info('Successfully loaded ${requests.length} group requests');
      return requests;
    } catch (e, stackTrace) {
